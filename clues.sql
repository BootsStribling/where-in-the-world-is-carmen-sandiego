...
SELECT code FROM country WHERE region ILIKE '%southern%europe%' ORDER BY population ASC LIMIT 1;
...
-- TODO: Write SQL query here
SELECT language FROM countrylanguage WHERE isofficial = true AND countrycode = 'VAT';

-- Clue #3: We have new news on the classes Carmen attended – our gumshoes tell us she's moved on to a different country, a country where people speak only the language she was learning. Find out which nearby country speaks nothing but that language.

-- TODO: Write SQL query here
SELECT * from country c LEFT JOIN countrylanguage l on c.code = l.countrycode where c.name ILIKE '%san%marino%';
...
-- TODO: Write SQL query here
SELECT * FROM city y LEFT JOIN country c ON y.countrycode = c.code WHERE c.code = 'SMR' AND y.name !='San Marino';
...
-- TODO: Write SQL query here
SELECT * FROM city y LEFT JOIN country c ON y.countrycode = c.code WHERE y.name ILIKE '%san%mari%' AND c.continent ILIKE '%south%america%';
...
-- TODO: Write SQL query here
SELECT y.name from city y LEFT JOIN country c ON y.id = c.capital WHERE c.code = 'BRA';
